...
const generateSlots = ({ type }: Pick<Slot, 'type'>) => {
  const slots = []
...
export const generateGarageSlots = () => {
  // get a random number of slots type that will be added
  const noOfSlotType = Math.floor(
    randomRange(1, Object.keys(slotRanges).length),
  )

  const slotTypes = Object.keys(slotRanges)
    .sort(() => Math.random() - 0.5)
    .slice(0, noOfSlotType)

  const slots = slotTypes.flatMap((type) =>
    generateSlots({ type: type as SlotType }),
  )
